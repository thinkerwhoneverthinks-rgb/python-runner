...
class RayBookJob:
    ...
    def run(self):
        ...
        if not self.default_token and not any(it.get("token") for it in self.items):
            ...
            self.log("Error: Authorization token is required to download books.", "error")
            return
        ...
        for idx, item in enumerate(self.items):
            ...
            active_token = book_token or self.default_token
    ...
    def _download_and_decrypt(self, asset_ref: str, token: str, output_raw: Path, book_idx: int, total_books: int) -> int:
        ...
        while not self.stop_requested:
            ...
            try:
                ...
                self.log(f"   [P.{page_num}] Network error: {e}. Retrying once...", "warning")
                time.sleep(1.0)
                response = requests.get(url, headers=headers, timeout=30)

            if response.status_code != 200:
                if page_num == 1:
                    raise ValueError(f"Server returned HTTP {response.status_code}. Verify your token or URL.")
